...
/* Body mimo filtr zůstanou na mapě jako matné tečky — kvůli přehledu o okolí. */
.mk.off{z-index:0}
.mk.off i{width:7px;height:7px;border-width:1px;opacity:.42;box-shadow:none}
.mk.off.top i{width:8px;height:8px;box-shadow:none}
.mk.sel{z-index:5}
.mk.sel.off i{opacity:1}
...
function apply(){
  ...
  P.forEach((p,i) => {
    p._on = pass(p);
    /* Značka na mapě nezmizí, jen zšedne — klepnout na ni jde pořád. */
    marks[i].el.classList.toggle('off', !p._on);
